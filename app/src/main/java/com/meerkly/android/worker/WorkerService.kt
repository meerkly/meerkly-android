 ...
 * the ProxyController; this service owns only two things — the
 * startForeground posture ("don't kill this process") and the ongoing
...
class WorkerService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        ServiceCompat.startForeground(
            ...
            WorkerNotification.build(this, graph.proxyController.state.value),
        ...
        if (scope == null) {
            scope = CoroutineScope(SupervisorJob() + Dispatchers.Main.immediate).also { s ->
                s.launch {
                    graph.proxyController.state.collect { state ->
                        ...
                        if (Build.VERSION.SDK_INT < 33 ||
                            ...
                            manager.notify(WorkerNotification.NOTIFICATION_ID, WorkerNotification.build(this@WorkerService, state))
                        }
                    }
                }
            }
